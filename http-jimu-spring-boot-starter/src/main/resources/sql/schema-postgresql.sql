...
ALTER TABLE http_jimu_config ADD COLUMN IF NOT EXISTS group_id VARCHAR(64);

CREATE INDEX IF NOT EXISTS idx_http_jimu_config_enable_job ON http_jimu_config(enable_job);
CREATE INDEX IF NOT EXISTS idx_http_jimu_config_group_id ON http_jimu_config(group_id);
...
ALTER TABLE http_jimu_pool ADD COLUMN IF NOT EXISTS steps_config TEXT;
...
COMMENT ON COLUMN http_jimu_config.group_id IS '所属分组ID';
...
COMMENT ON COLUMN http_jimu_pool.steps_config IS '连接池公共步骤JSON';
...
COMMENT ON TABLE http_jimu_group IS 'HTTP积木接口分组表';
COMMENT ON COLUMN http_jimu_group.id IS '主键ID';
COMMENT ON COLUMN http_jimu_group.code IS '分组编码';
COMMENT ON COLUMN http_jimu_group.name IS '分组名称';
COMMENT ON COLUMN http_jimu_group.description IS '分组描述';
COMMENT ON COLUMN http_jimu_group.steps_config IS '分组公共步骤JSON';
COMMENT ON COLUMN http_jimu_group.create_time IS '创建时间';
COMMENT ON COLUMN http_jimu_group.update_time IS '更新时间';
